...
3. 全量切换：100% 流量切到 v2，**旧版 v1 保留作回滚点（不立即删除），观察期（24-72h）确认稳定后手动删除**

> 为什么旧版不立即删：灰度刚切完就删旧版，如果新版本 10 分钟后出问题，回滚点就没了。所以旧版保留 0 流量跑着，观察期确认稳定后再删。Service 和 VirtualService 不需要删——Service 是稳定入口（app: backend），VirtualService 的 full 版本已覆盖 header 版本，要清理的只有旧版 Deployment，且在观察期之后。
