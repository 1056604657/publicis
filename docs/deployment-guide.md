...
3. 全量切换：100% 流量切到 v2
4. **角色轮换**（为下次灰度做准备）：把 base 的 backend（blue 稳定版）更新成新镜像 → 恢复 header 路由（流量默认走 blue，此时 blue 已是新版本）→ 最后删除临时 green

> **为什么角色轮换**（面试重点）：blue 是常驻稳定版（base 的 backend），green 是每次发布临时创建的。灰度验证通过后，blue 直接升级到新版本、green 删除——这样 blue 永远是稳定版、green 永远是待验证的新版，每次发布循环复用同一套角色，不会同名冲突。顺序必须是「先更新 blue → 恢复路由（流量切回 blue）→ 最后删 green」，避免流量中断。
>
> Service 和 VirtualService 不需要删：Service 是稳定入口（app: backend），VirtualService 在角色轮换时恢复 header 路由版本。
